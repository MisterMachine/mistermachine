...
require_once('classes/sidegear.php'); // Class for setting up environment: custom post types, featured images, etc
...
/**
* sg_add_site_styles function.
* Add site styles to the head of the theme
* @access public
* @return void
*/	
function sg_add_site_styles()
{
	if ( ! is_admin() && ! sg_is_login_page()) {
		// Responsive:
...
function sg_submit_newsletter_signup() {
...
/**
 * sg_show_admin_bar function.
 * Control where admin bar is displayed.
 * @access public
 * @return void
 */
function sg_show_admin_bar() {
...
add_action('wp_print_styles', 'sg_add_site_styles');

// Add Theme Scripts
add_action('wp_print_scripts', 'sg_add_site_scripts');

// Hide admin bar
add_filter( 'show_admin_bar', 'sg_show_admin_bar' );
